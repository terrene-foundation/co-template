 ...
 * Domain-agnostic — works with any CO workspace.
 */
 ...
 *
 * Generic CO workspace phases (domain repos may override):
 * - Has 05-codify/ or .claude/agents/project/ files -> phase 05-codify
 * - Has 04-validate/ with files -> phase 04-validate
 * - Has todos/completed/ or src/ or apps/ or 03-drafts/ -> phase 03-implement
 * - Has todos/active/ or 02-plans/ with files -> phase 02-plan
...
function derivePhase(workspacePath, cwd) {
  // Check for phase 05 artifacts (codify / project-level agents/skills)
  if (cwd) {
  ...
  if (dirHasFiles(path.join(workspacePath, "05-codify"))) {
    return "05-codify";
  }

  // Check for phase 04 artifacts (validation / red-team)
  if (dirHasFiles(path.join(workspacePath, "04-validate"))) {
    return "04-validate";
  }

  // Check for phase 03 activity (implementation / drafts)
  ...
  // Check for phase 02 (planning / todos)
...
function findAllSessionNotes(cwd) {
  ...
  try {
    ...
    for (const entry of entries) {
      if (!entry.isDirectory() || entry.name === "instructions" || entry.name.startsWith("_")) continue;
