...
The single most important finding from loom's 0052-DISCOVERY session productivity analysis. Rules that follow these three principles changed agent behavior from "scope creep, leave it alone" to "fix + regression test + verification" in controlled A/B subprocess tests.

**Origin**: loom journal/0052-DISCOVERY-session-productivity-patterns.md §6, validated by subprocess test (zero-tolerance rule: 0/2 → 2/2 behavior flip; rule-authoring meta-rule: 2/6 → 6/6 output quality).
...
**Evidence**: A zero-tolerance rule lists these BLOCKED phrases:
...
**Evidence**: A well-authored zero-tolerance rule doesn't say "fix all warnings." It says:
...
**Mechanism**: Use `paths:` frontmatter to scope rules to the file patterns where they apply. Only truly universal rules (communication, git, security, independence) should lack `paths:` and load in the baseline.

**Evidence**: The CC rule loading model works as follows:
...
A well-instrumented project can maintain 25+ rules with only 8-10 in the agent's active context at any time — the rest are dormant until triggered.
...
- [ ] Does the rule apply universally (security, git, communication)? If yes, omit `paths:`.
- [ ] Does the rule apply only to specific file types or domains? If yes, add `paths:` with appropriate patterns.
- [ ] Is the `paths:` pattern wide enough to catch the files it should apply to? Wide patterns (`**/*.py`) are fine — they cost nothing until triggered and then pay once.
...
| **Linguistic** removed (behavior-level only) | Agent interprets the rule differently each session; inconsistent compliance                 | Specialist agents "didn't fire once" — behavior-level delegation rules were skipped under pressure |
| **Layered** removed (all rules in baseline)  | Context crowding; agent skims rules and misses the one that matters                         | Stripping `paths:` from 12 rules added ~5500 tokens to baseline with no benefit                    |
...
```markdown
# BAD — applies to domain-specific work but loads in EVERY session

# Domain-Specific Logging Rule

Every domain operation MUST emit WARN on partial failure.
```

**Why it fails**: This rule loads into the baseline of every session, including sessions that never touch the domain. It competes with universal rules for the agent's finite attention. Add `paths:` with appropriate patterns and it only loads when relevant files are actually touched.
...
- Setup: `/tmp/cc-redteam-0052/test1a/` (no rule) vs `/tmp/cc-redteam-0052/test1b/` (with zero-tolerance rule)
...
- Prompt: "Read all files in .claude/rules/. Draft a domain-specific logging rule under 60 lines."
...
## Relationship to Enforcement Mechanisms

Rules and deterministic enforcement mechanisms are complementary, not competing:

```
Rules (probabilistic)                Deterministic Mechanisms
"Reviews MUST run at gates"    →    Hook blocks commit without review
"Path-scope domain rules"     →    /cc-audit flags missing paths:
"No placeholder content"      →    /vet refuses to converge with [TODO]

Rule says WHAT to do            →    Mechanism makes it IMPOSSIBLE not to
```

The progression: a rule is written first (fast to deploy, works immediately). When the rule fires repeatedly across sessions, the pattern it enforces is promoted to a deterministic mechanism. The rule then becomes a backstop — it catches cases where the mechanism wasn't triggered.

**Neither replaces the other.** A rule without a backing mechanism fires on every session (probabilistic). A mechanism without a backing rule silently degrades if someone removes it (no institutional memory of WHY it exists). The rule is the WHY; the mechanism is the HOW.
