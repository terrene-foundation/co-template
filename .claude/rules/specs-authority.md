...
The project decides its own file structure. Spec files are named after what they describe (components, features, modules, user needs), NOT after CO process stages.
...
When domain truth changes during any phase, the relevant spec file MUST be updated immediately — not batched at phase end. If a decision during `/execute` changes a contract, the relevant spec file is updated in the same action.

```
# DO — update spec when the truth changes
1. Execute task that changes a method signature
2. Immediately update the relevant spec with new signature
3. Continue execution

# DO NOT — batch updates for later
1. Execute task that changes signature
2. Execute 5 more tasks
3. "I'll update specs later" → specs are now stale for tasks 2-6
```
...
When execution deviates from a spec (different approach, technology, or user-observable behavior), the agent MUST: (a) update the spec file with the new truth, (b) log the deviation with rationale, and (c) flag user-visible changes for approval.
...
# DO NOT — silently execute differently
...
- "The spec said X, and X is done" (when approach differs from spec)
- "This is an execution detail, not a spec change"
...
- Organize spec files by CO process stages (intent, decisions, progress) — duplicates workspaces/ artifacts
- Read the entire `specs/` directory at any phase gate — `_index.md` exists for selective reads (exception: `/vet` and `/codify` may read all specs for audit and knowledge extraction)
- Treat specs as optional documentation — specs prevent the 6 drift failure modes

**BLOCKED:** "Specs can be written after execution", "The output is the spec", "Plans already capture this, specs are redundant", "Updating specs for this minor change is overkill"
